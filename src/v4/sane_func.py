"""
SANE with Functional AE Training
==================================
Key change: Train the SANE autoencoder with functional loss FROM THE START,
not just MSE. This forces the latent space to preserve functional information.

Stage 1: SANE AE with MSE + functional loss (joint from epoch 1)
Stage 2: Latent hypernetwork with functional loss (decoder frozen initially)
...
def run():
    ...
    # ===================================================================
    # STAGE 1: SANE AE with MSE + FUNCTIONAL loss from the start
    # ===================================================================
    print(f"\n{'='*60}")
    print("STAGE 1: SANE AE with functional loss (MSE + CE joint)")
    print(f"{'='*60}")

    sane = SANEAutoencoder(token_dim=token_size, d_model=256, d_z=d_z,
                           n_heads=8, n_layers=4, dropout=0.1).to(device)
    print(f"SANE params: {sum(p.numel() for p in sane.parameters()):,}")
    ...
    no_improve = 0
    func_weight = 0.0  # warm up functional loss
    ...
    for epoch in range(1, 501):
        ...
        # Warm up functional loss: 0 for first 50 epochs, then ramp up
        if epoch <= 50:
            func_weight = 0.0
        elif epoch <= 100:
        ...
        for start in range(0, n_train, BS):
            ...
            for i in batch_idx:
                ...
                # Functional loss: decode reconstructed tokens, evaluate
                if func_weight > 0:
                    recon_flat = detokenize_to_flat(recon.squeeze(0), bm.squeeze(0), cfg.target)
        ...
        # Val: functional accuracy of reconstructed weights
        sane.eval()
        vc = vt = 0
        with torch.no_grad():
            for idx in range(min(30, n_train)):
                _, recon = sane(all_tokens[idx:idx + 1], ref_positions.unsqueeze(0))
                recon_flat = detokenize_to_flat(recon.squeeze(0), all_masks[idx], cfg.target)
                recon_w = recon_flat * w_std.squeeze(0) + w_mean.squeeze(0)
                classes = train_classes[idx]
        ...
        if epoch % 25 == 0 or epoch == 1:
            ...
            print(f"  Epoch {epoch:3d} | Loss: {el/nb:.4f} | Train: {ta:.4f} | Val Recon Acc: {va:.4f} | fw: {func_weight:.2f} | Best: {best_val_acc:.4f}", flush=True)
    ...
    for epoch in range(1, 151):
        ...
        for start in range(0, n_train, 8):
            ...
            for i in batch_idx:
                ...
                z_pred = hypernet(protos)
                recon_tokens = sane.decode(z_pred.unsqueeze(0), ref_positions.unsqueeze(0)).squeeze(0)
                gen_w = detokenize_to_flat(recon_tokens, all_masks[0], cfg.target) * w_std.squeeze(0) + w_mean.squeeze(0)

                logits = differentiable_forward(gen_w, imgs, cfg.target)
                bl = bl + F.cross_entropy(logits, labs)
    ...
    torch.save({"sane": best_sane_state, "hypernet": best_hn_state,
               ...
               Path("data/v4b/checkpoints/sane_func.pt"))

    # ===================================================================
    # STAGE 4: Eval across 0/1/2/3 unseen classes
    # ===================================================================
    print(f"\n{'='*60}")
    print("STAGE 4: Eval across novelty levels")
    print(f"{'='*60}")
