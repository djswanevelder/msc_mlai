...
% Weight space learning encompasses two broad classes of tasks: generative and discriminative. In generative applications, researchers aim to model the underlying distribution of neural network weights, conditioned on additional information or reference models, \(P(W \mid \dots)\). Sampling from this distribution enables the generation of entirely new model weights.  

% For example, \cite{schurholt2022hyperrepresentationsgenerativemodelssampling} used an autoencoder to generate hyper-representations of multiple model zoos and conditioned on these representations to produce new model weights. Building on this, \cite{pmlr-v235-schurholt24a} introduced the Sequential Autoencoder for Neural Embeddings (SANE), which improves scalability by tokenising model weights and embedding them sequentially, enabling generative modelling for much larger architectures.  

% Other methods focus on conditioning the weight distribution on dataset or performance information. \cite{bedionita2025instructionguidedautoregressiveneuralnetwork} employed a Vector Quantised Variational Autoencoder (VQ-VAE) to model \(P(W \mid \mathcal{D})\), learning latent representations that allow generation of new weights for specific tasks. Similarly, \cite{meynent2025structureenoughleveragingbehavior} modelled \(P(W \mid R)\) using a contrastive objective that minimises differences in both model outputs and weights between original and reconstructed networks, effectively embedding behavioural information into the latent space.  

% In discriminative applications, pre-trained network weights—often collected into a model zoo \cite{schurholt2022modelzoosdatasetdiverse}—serve as input to predict meta-information about the original models \cite{unterthiner2021predictingneuralnetworkaccuracy}. Weight space representations are typically evaluated via a simple multi-layer perceptron (MLP) tasked with predicting such meta-information from the embeddings. Common metrics include final model performance and generalisation gap. These discriminative tasks demonstrate that weight embeddings capture meaningful training characteristics, providing both a measure of representation quality and practical predictive insights.
...
The weight space \(W\) of a neural network is formally defined as the high-dimensional Euclidean space \(\mathbb{R}^N\) spanned by the \(N\) learnable parameters—the weights and biases—of the specified architecture. A specific point \(\mathbf{w} \in W\) represents a complete instantiation of the model, mapping input data \(\mathbf{x}\) to output predictions \(f(\mathbf{x}, \mathbf{w})\).  
...
Autoencoders are neural network architectures designed for unsupervised representation learning, as illustrated in Figure~\ref{fig:autoencoder}. Their primary goal is to learn a compressed, informative latent representation of input data by training the system to reconstruct the original input after it passes through a lower-dimensional bottleneck layer. The full architecture consists of an encoder and a decoder. Crucially, the layers within these components typically employ non-linear activation functions, enabling the model to perform complex, non-linear mappings from the high-dimensional input space to the latent space. This capability is essential for capturing intricate, hierarchical structures within the data that linear dimensionality reduction techniques cannot access.
...
\begin{figure}[!t]
    ...
    \caption{Contrastive encoder pulling similiar data pairs $(x,x^+)$ together in latent space and pushing dissimilar pairs $(x,x^-)$ apart \cite{foundationsCVbook}.}
    \label{fig:contrastive}
\end{figure}

\subsection{NT-Xent Loss}
\label{sec:ntxne_loss}
A widely used objective for contrastive learning is the Normalised Temperature-Scaled Cross Entropy (NT-Xent) loss introduced in \cite{agren2022ntxentlossupperbound}. This loss formulates the contrastive task as a classification problem over positive and negative pairs within a batch.  
