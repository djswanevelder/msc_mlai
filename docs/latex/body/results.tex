...
\label{sec:weights}
To evaluate the efficacy of the weight autoencoder, we examine the evolution of the mean squared error (MSE) during training. Figure \ref{fig:weight_encoder_performance} plots the MSE observed on both the training and validation sets following PCA projection of the model weights. The training error consistently decreases over epochs, eventually reaching a plateau, while the validation error follows a similar trajectory. This parallel behaviour suggests that the model is not overfitting to the training data and is capable of generalizing to unseen weight representations.
